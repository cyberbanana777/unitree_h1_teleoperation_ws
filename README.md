...
*   **`ukt_library/`**: python3-библиотека, которая облегчает работу с УКТ
...
##### `limits.py`
- **`LIMITS_OF_JOINTS_UKT`** - словарь: ключ - индекс звена УКТ, значение - пределы УКТ для звена данного.
##### `mapping.py`
...
##### `normalize_values.py`
- **`JOINT_CONVERSION_COEFFICIENTS`** - словарь: ключ - индекс УКТ, значение - коэффициент нормализации который приводит координату или в систему СИ, или в нормированные единицы.
##### `utils.py`
- **`map_range(value: float, in_min: float, in_max: float, out_min: float, out_max: float) -> float`** - функция, которая производит линейное преобразование значение из 1-ой системы координат в 2-ую систему координат. Для корректной работы необходимы следующие аргументы:
	- `value` - значение, которое необходимо перевести в другую систему координат (из 1-ой во 2-ую);
	- `in_min` - наименьшее значение в 1-ой системе координат
	- `in_max` - наибольшее значение в 1-ой системе координат
	- `out_min` - наименьшее значение в 2-ой системе координат
	- `out_max` - наибольшее значение в 2-ой системе координат
	Возвращает функция значение координаты во 2-ой системе координат, которому соответствует значение до преобразования в 1-ой системе координат.
- **`convert_from_ukt_to_unitree_h1(data: list) -> dict`** - функция, которая производит линейное преобразование значение из системы координат УКТ в систему координат робота Unitree H1. Для корректной работы необходимы следующие аргументы:
	- `data` - список, объектами которого являются словари, каждый из которых содержит информацию об угле звена, его имени, и. (Это не "голые" данные. Они предварительно получены по ключу "slaves" из "голых" данных, полученных с УКТ )
	Функция возвращает словарь с целевыми значениями для углов сочленений робота Unitree H1 в формате: ключ - индекс звена Unitree H1, значение - соответствующая координата.
- **`convert_from_ukt_to_rad(data: list) -> dict`** - функция, которая производит линейное преобразование значение из системы координат УКТ в систему СИ или нормированные единицы (пальцы - норм. единицы, остальные - СИ). Для корректной работы необходимы следующие аргументы:
	- `data` - список, объектами которого являются словари, каждый из которых содержит информацию об угле звена, его имени, и. (Это не "голые" данные. Они предварительно получены по ключу "slaves" из "голых" данных, полученных с УКТ )
	Функция возвращает словарь с  значениями для углов в радианах или нормированных единицах в формате: ключ - индекс звена УКТ , значение - соответствующая координата в СИ или норм. единицах.
